...
class PhotosViewModelTest {
  ...
  fun `test loadPhotos correctly load the photos`() = runTest {
      ...
      assertEquals("Successfully loaded photos", photosViewModel.uiState.value.toastMessage)
  }
    ...
    fun `test loadPhotos correctly load the photo`() = runTest {
        ...
        assertEquals("Successfully loaded photo", state.value.toastMessage)
    }
  ...
  fun `test savePhotos correctly adds new uris to the repository`() = runTest {
    ...
    @Test
    fun `savePhotos correctly sets toast message when getTrip throws exception with one photo`() = runTest {
        ...
        // getTrip will fail
        `when`(mockRepo.getTrip(fakeTripNoPhoto.uid)).thenThrow(RuntimeException("Could not get the trip"))
