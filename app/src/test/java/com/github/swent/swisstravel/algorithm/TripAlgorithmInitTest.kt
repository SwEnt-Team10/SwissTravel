...
class TripAlgorithmInitTest {
  ...
  fun setup() {
    ...
    mockkConstructor(
        com.github.swent.swisstravel.algorithm.orderlocationsv2.DurationMatrixHybrid::class)
    mockkConstructor(
        com.github.swent.swisstravel.algorithm.orderlocationsv2.ProgressiveRouteOptimizer::class)
  }
  ...
  fun `init adds CHILDREN_FRIENDLY preference when children present`() {
    // Given: 1 child
    val settings =
        TripSettings(travelers = TripTravelers(adults = 2, children = 1), preferences = emptyList())
  ...
  fun `init adds INDIVIDUAL preference when single adult and no children`() {
    ...
    val settings =
        TripSettings(travelers = TripTravelers(adults = 1, children = 0), preferences = emptyList())
  ...
  fun `init adds GROUP preference when 3+ adults and no children`() {
    ...
    val settings =
        TripSettings(travelers = TripTravelers(adults = 3, children = 0), preferences = emptyList())
    ...
    val settings =
        TripSettings(
            ...
            preferences =
                listOf(
                    Preference.MUSEUMS,
                    Preference.URBAN) // Provide basic prefs to avoid auto-fill logic
            )
  ...
  fun `init fills all preferences when input is empty`() {
    ...
    val settings =
        TripSettings(travelers = TripTravelers(adults = 2, children = 0), preferences = emptyList())
  ...
  /**
   * Helper to extract the TripSettings using reflection, allowing us to verify the state inside the
   * private SelectActivities instance.
   */
