...
class RandomTripGeneratorTest {
  ...
  fun `generator uses provided arrival location as start`() {
    val arrivalLocation = Location(name = "Genève", coordinate = Coordinate(46.204391, 6.143158))
    val settings =
  ...
  fun `intermediate stops are distinct from start and end`() {
    ...
    repeat(10) {
      ...
      assertFalse(
          "Intermediate stops should not contain start", intermediateNames.contains(start.name))
