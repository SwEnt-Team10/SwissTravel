...
class AuthRepositoryFirebaseTest {
    ...
    fun signInWithGoogle_Success() = runTest {
        coEvery { mockAuth.signInWithCredential(mockFirebaseCredential) } returns Tasks.forResult(mockAuthResult)
    ...
    fun signInWithGoogle_Failure_WrongCredentialType() = runTest {
        ...
        assertEquals("Login failed: Credential is not of type Google ID", result.exceptionOrNull()?.message)
    }
    ...
    fun signInWithGoogle_Failure_UserNull() = runTest {
        ...
        assertTrue(result.isFailure)
        assertEquals("Login failed: Could not retrieve user information", result.exceptionOrNull()?.message)
    }

    @Test
    fun signInWithGoogle_Failure_FirebaseSignInFails() = runTest {
        val exception = Exception("Firebase error")
        coEvery { mockAuth.signInWithCredential(mockFirebaseCredential) } returns Tasks.forException(exception)
