...
open class TripAlgorithm(
     ...
     * @property protectedActivities List of activities that should not be removed during optimization.
     * @property activityBlacklist List of activity names to exclude from fetching/selection.
     * @property allFetchedLocations List of locations for which activities have already been fetched (to avoid redundancy).
     */
    ...
    suspend fun computeTrip(
        ...
        val activities =
            Activities(
                ...
                allActivities = (grandTourActivities + selectionParams.protectedActivities).toMutableList(),
        ...
        try {
            ...
            val optimizedRoute =
                computeInitialRoute(
                    tripSettings, enhancedTripProfile, activities, selectionParams.activityBlacklist, onProgress)
     ...
     * added here.
     * @param activityBlacklist A list of activity names to exclude from selection and intermediate stops.
     ...
     * @param activityBlacklist A list of activity names to exclude when generating intermediate stops.
    ...
    private suspend fun completeSchedule(
        ...
        while (index < maxIndex) {
            ...
            finalSchedule = tryAddingCityActivities(enhancedTripProfile, activities, selectionParams.activityBlacklist, finalSchedule)
            ...
            finalSchedule =
                tryAddingCity(enhancedTripProfile, selectionParams.activityBlacklist, activities, finalSchedule, selectionParams.allFetchedLocations)
     ...
     * @param enhancedTripProfile The enhanced trip profile.
     * @param activities The mutable [Activities] state object.
     * @param activityBlacklist A list of activity names to exclude (e.g., previously disliked city visits).
     * @param originalSchedule The current trip schedule.
    ...
    private suspend fun tryAddingCityActivities(
        ...
        while (added && index < maxIndex) {
            ...
            if (totalTimeNeededHours > 0) {
                added = addCityActivity(enhancedTripProfile, activities, activityBlacklist, totalTimeNeededHours)
                if (added) {
     ...
     */
    private fun findValidCity(enhancedTripProfile: EnhancedTripProfile, allFetchedLocations: List<Location>): CityConfig? {
        // 1. Filter out cities that are already visited/near preferred locations
        val candidates =
            swissMajorCities.filter { cityConfig ->
                val isNearPreferred = enhancedTripProfile.newPreferredLocations.any { location ->
                    location.coordinate.haversineDistanceTo(cityConfig.location.coordinate) <=
                            cityConfig.radius
                }
                val isNearFetched = allFetchedLocations.any { location ->
                    location.coordinate.haversineDistanceTo(cityConfig.location.coordinate) <=
                            cityConfig.radius
                }
